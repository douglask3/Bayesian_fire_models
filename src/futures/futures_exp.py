...
def plot_projection_images(ax, sample_type = "mean", run = "Evaluate.csv", obs = None, 
                           cmap = 'gradient_hues', *args, **kw):
    BAs = open_BA_projections(sample_type = sample_type, run = run, *args, **kw) 
    BAs = np.array(BAs).transpose(1, 0, 2, 3)
    #BAs /= BAs[:,:,:,0:10].mean(axis = 3, keepdims=True)
    BAs.sort(axis = 3)
    image_data = np.log(BAs.reshape(-1, BAs.shape[3]) + 0.000001)
    img = ax.imshow(image_data, aspect='auto', cmap='viridis')
    plt.colorbar(img, label='Value')
    set_trace()
            

def plot_projection_likihood(ax, sample_type = "mean", run = "Evaluate.csv", obs = None, 
                             cmap = 'gradient_hues', *args, **kw):
    print(run)
    BAs = open_BA_projections(sample_type = sample_type, run = run, *args, **kw) 
...
def plot_future_projections(obs_path, mod_path, Region_name, periods, models, 
    ...
    fig1, axes1 = plt.subplots(3, 3, figsize=(10, 7), sharex=True, layout='constrained')
    fig2, axes2 = plt.subplots(3, 3, figsize=(8, 8), layout='constrained')
    ...
    for sample_type, sample_name in zip(sample_types, sample_names):
        ...
        for run, title, cmap in zip(runs, plot_names, cmaps):
            ax = axes1[axi]
            obs = plot_projection_likihood(ax, sample_type, run, obs, cmap, 
            ...
            ax = axes2[axi]
            plot_projection_images(ax, sample_type, run, obs, cmap, models, periods, mod_path)
            axi += 1
