...
def plot_map(ax, data, title, cmap):#, vmin=None, vmax=None):
    ...
    data.plot(ax=ax, cmap=cmap, transform=ccrs.PlateCarree(), cbar_kwargs={'shrink': 0.5})
...
def plot_isimip_drivers_for_region(region, pdf):
    print(region)
    base_dir = "data/data/driving_data2425/" + region + "/isimp3a/obsclim/GSWP3-W5E5/period_2000_2019/"
    # Load data
    datasets = {}
...
with PdfPages("fire_analysis_all_regions.pdf") as pdf:
    for region in regions:
        plot_isimip_drivers_for_region(region, pdf)
