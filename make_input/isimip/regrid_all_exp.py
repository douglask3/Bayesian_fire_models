...
def make_variables_for_year_range(year, process, dir, temp_dir_jules, dataset_name, filenames,
    ...
    print(temp_out)
    def open_variable(varname, MinusYr = False, jules = False):
        filename = filenames[varname]
        
        if jules:
            fdown = dir + filename + "1901_2019.nc"
            fname = temp_dir_jules + fdown.split('/')[-1]
            dir_var = temp_dir_jules
        else:
            dir_var = dir
    ...
    def cal_cover(cover_vars, name):
        print(name)
        dat = open_variable(cover_vars[0], jules = True)
...
def process_clim_and_jules(process_jules, url_jules_3a, file_jules, temp_dir_jules, 
                           process_clim, dir_clim, years,
                           *args, **kw):
    def process(process, dir):
        [make_variables_for_year_range(year, process, dir, temp_dir_jules,
                                       *args, **kw) for year in  years]
    set_trace()
    process(process_jules, url_jules_3a + file_jules)
...
def for_region(subset_functions, subset_function_argss, vcf_region_name, region_name = None):   
    ...
    url_jules_3a = "https://files.isimip.org/ISIMIP3a/OutputData/biomes/JULES-ES-VN6P3/gswp3-w5e5/historical/"
    dir_clim = "/hpc//data/d00/hadea/isimip3a/InputData/climate/atmosphere/obsclim/GSWP3-W5E5/gswp3-w5e5_obsclimfill_"
    file_jules = "jules-es-vn6p3_gswp3-w5e5_obsclim_histsoc_default_pft-"  
    temp_dir_jules = '/scratch/dkelley/Bayesian_fire_models/temp/isimip/'
    process_clim_and_jules(process_jules, url_jules_3a, file_jules, temp_dir_jules, 
                           process_clim, dir_clim, years,
...
if __name__=="__main__":
    ...
    subset_functions_main = [constrain_natural_earth]
    subset_function_argss_main = [{'Country': 'Wales', 'shapefile_name': 'admin_0_map_units'}]
    for_region(subset_functions_main, subset_function_argss_main, None, region_name = 'Wales')
    
    subset_functions_main = [constrain_natural_earth]
    subset_function_argss_main = [{'Country': 'United Kingdom'}]
    for_region(subset_functions_main, subset_function_argss_main, None, 
               region_name = 'United_kingdom')
    set_trace()
    
    subset_functions_main = [constrain_cube_to_shapefile]
    subset_function_argss_main =[{'shapefile': 'data/LIM_RAISG_buffer/'}]
    for_region(subset_functions_main, subset_function_argss_main, None, region_name = 'Amazon_basin')
    set_trace()
