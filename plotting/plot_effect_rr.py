...
def effect_ratio_and_rr_over_range(factual_flat, counterfactual_flat, 
    ...
    def for_x(x):
        test = factual_flat > x
        er = np.percentile(effect_ratio[test], [5, 10, 50, 90, 95])
    ...
    # Left axis labels
    ax.set_xlabel('X')
    ax.set_ylabel('Response', color='tab:blue')
    ...
    ax2 = ax.twinx()
    ax2.plot(xs, risk_ratio, color='tab:red', label='Risk ratio', linewidth=2)
    ...
    plt.title('Response and Risk Ratio')
    plt.tight_layout()
    #plt.show()
